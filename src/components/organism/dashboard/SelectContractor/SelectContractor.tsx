...
import {
  getSelectedContractorList,
  setAwardedContractor,
} from '@/services/users';
...
function SelectContractor() {
  ...
  useEffect(() => {
    if (router.isReady && serviceRequestId) {
      getSelectedContractorList(serviceRequestId as string)
        .then(res => {
  ...
  const handleSubmit = async () => {
    if (selectedItem) {
      setIsPosting(true);
      setAwardedContractor(selectedItem.id, serviceRequestId as string)
        .then(res => {
  ...
  return (
    <Flex gap="8" flex={1} overflow={'hidden'}>
          ...
          <CardHeader>
            <VStack gap={'20px'} alignItems={'flex-start'}>
              <Text sx={{ ...cardStyle.headerTitle, ...cardStyle.guideTitle }}>
                {t('portal_select_contractor_title')}
              </Text>
              <Text sx={cardStyle.headerSubtitle}>
                {t('portal_select_contractor_description')}
              </Text>
            </VStack>
          </CardHeader>
          <Divider />
      ...
      <InfoModal
        ...
        title={t('portal_select_contractor_success_popup_title')}
        info={t('portal_select_contractor_success_popup_description')}
